...
CString CColorPickerCB::GetSelectedColorName()
{
	...
	if (iSelectedItem == CB_ERR)							// If Nothing Selected
		return (m_sColorName = _T(""));				// Return Nothing (Not "Black!")
...
void CColorPickerCB::OnDropdown() 
{
	TRACE(_T("in CColorPickerCB::OnDropdown\n"));
	
}

void CColorPickerCB::OnLButtonDown(UINT nFlags, CPoint point) 
{
	TRACE(_T("in CColorPickerCB::OnLButtonDown\n"));
